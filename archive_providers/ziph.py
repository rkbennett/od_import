...
class ziph(object):
    ...
    def __init__(self, data: bytes, url_base: str, path_cache: list=[], config: None=None):
        """
        Description:
    ...
    def extractor(self, url: str, path: str="", path_cache: list=[], cache_update: bool=False, config: None=None) -> bytes:
        """
        Description:
        ...
        if path in self.path_cache:
            return b""
        return self.zip_bytes.open(url.replace(self.url, "").lstrip("/"), 'r').read()